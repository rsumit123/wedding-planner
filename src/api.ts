...
export type ApiEvent = { id: number; slug: string; name: string; date: string; time_note: string; venue: string };
export type GuestSummary = { total: number; bride_total: number; groom_total: number; events: Array<ApiEvent & { guest_count: number }> };
export type ApiVendor = { id: number; name: string; category: string; side: 'bride' | 'groom' | 'both'; amount: number; paid_amount: number };
...
export const api = {
  ...
  addEvent: (name: string, date: string, timeNote: string, venue: string) => request<ApiEvent>('/events', { method: 'POST', body: JSON.stringify({ name, date, time_note: timeNote, venue }) }),
  updateEvent: (id: number, name: string, date: string, timeNote: string, venue: string) => request<ApiEvent>(`/events/${id}`, { method: 'PATCH', body: JSON.stringify({ name, date, time_note: timeNote, venue }) }),
  ...
  addVendor: (name: string, category: string, side: 'bride' | 'groom' | 'both', amount: number, paidAmount: number) => request<ApiVendor>('/vendors', { method: 'POST', body: JSON.stringify({ name, category, side, amount, paid_amount: paidAmount }) }),
  updateVendor: (id: number, name: string, category: string, side: 'bride' | 'groom' | 'both', amount: number, paidAmount: number) => request<ApiVendor>(`/vendors/${id}`, { method: 'PATCH', body: JSON.stringify({ name, category, side, amount, paid_amount: paidAmount }) }),
};
