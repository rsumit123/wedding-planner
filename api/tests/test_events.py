...
def test_organiser_managed_events_feed_public_invite_and_all_event_guests():
    with TestClient(app) as client:
        ...
        created = client.post('/events', json={'name': 'Sangeet night', 'date': '2026-11-29', 'time_note': '7 PM onwards', 'venue': 'Family courtyard', 'side': 'bride'})
        assert created.status_code == 200
        event = created.json()
        assert event['venue'] == 'Family courtyard'
        assert event['side'] == 'bride'
        ...
        updated = client.patch(f'/events/{event["id"]}', json={'name': 'Sangeet night', 'date': '2026-11-29', 'time_note': '8 PM onwards', 'venue': 'New courtyard', 'side': 'both'})
